...
export default function absolutify (str, url) {
  if (typeof url === 'function') return iterate(str, url)
  return str
    .replace(rx, '$1' + url + '/$6') // Inject the URL into the attribute
...
const rx = /((href|src|codebase|cite|background|action|profile|formaction|icon|manifest|archive)=["'])(?!(http|https|ftp|file|filesystem|gopher|ws|wss|about|blob|data|mailto):|(\/\/))((?:\/)?([^'"]+))/g // jshint ignore:line
...
const iterate = function (str, iterator) {
  return str.replace(rx, function () {
    const url = arguments[6]
